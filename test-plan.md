...
| `KdTreeTest::BuildHasLimitedHeight` | `gengeopop::KdTree::Build` | Build the KdTree with a number of points and verify the height and size. |
| `KdTreeTest::RangeQuery` | `gengeopop::KdTree::Query` | Build the KdTree with a number of points, perform a query and verify the resulting points. |
| `KdTreeTest::Insert` | `gengeopop::KdTree::Insert` | Insert a point and verify if it is succesfully inserted. |
| `KdTreeTest::HigherDimensional` | `gengeopop::KdTree::Apply` | Insert a number of points in 4 dimensions and verify if the operations still work. |
| `KdTreeTest::NonArithmeticDimension` | `gengeopop::KdTree::Insert` | Verify the tree with non-arithmetic dimensions |
