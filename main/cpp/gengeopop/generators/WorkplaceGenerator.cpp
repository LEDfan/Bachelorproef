...
void WorkplaceGenerator::apply(std::shared_ptr<GeoGrid> geoGrid, GeoGridConfig& geoGridConfig) {
         ...
         * 3. calculate the amount of working people at each location = #residents + #incoming commuting people - #outgoing commuting people
         * 4. use the last information for the distribution
         * 5. assign each workplace to a location
         */

        auto amountOfEmployees = static_cast<int>(std::floor(geoGridConfig.populationSize * geoGridConfig.fraction_Workables_WhichAreActive));
        auto amountOfWorkplaces = static_cast<int>(std::ceil(amountOfEmployees / 20.0)); // TODO magic constant

        std::vector<double> weights; // = for each location #residents + #incoming commuting people - #outgoing commuting people

        for (const std::shared_ptr<Location> &loc : *geoGrid) {
                double amountOfWorkingPeople =
                        loc->getPopulation() * geoGridConfig.fraction_Workables_WhichAreActive
